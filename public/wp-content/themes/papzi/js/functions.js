...
( function( $ ) {
	...
	function _papzi_accordion_menu(){	
	...
	function _headercategories(){
		...
		$('.papzi-close',$menu_categories).on( "click", function() {
			$menu_categories.removeClass('active');
			return false;
		});		
		//Navigation right			
	...
	function _canvasrightNavigation(){
		...
		$('.papzi-close',$wpbingo_menu_right).on( "click", function() {
			$wpbingo_menu_right.removeClass('active');
			return false;
		});		
		//Navigation right			
	...
	function _sticky_nextproduct(){
		...
		if( $(".prev_next_buttons",$parent).length > 0 ){
			...
			_window.scroll(function() {
				var scroll_top = _window.scrollTop();
				if($(".single_add_to_cart_button",$parent).length > 0){
					var offset_top = $(".single_add_to_cart_button",$parent).offset().top;
					var distance   = (offset_top - scroll_top);
					if ( distance <= 0) {
					...
					var offset_top = $(".product-stock",".single-product").offset().top;
					var distance   = (offset_top - scroll_top);
					if ( distance <= 0) {
						$('.prev_next_buttons',$parent).addClass('active');
					}else{
		...
		$(".input-search",$element).on("keydown", function() {
			...
			if(character.length >= 2){
				...
				$.ajax({
					...
					dataType: 'json',
					data: {
						action : "papzi_search_products_ajax",
						character : character,
						limit : limit,
						category : category
					},
	...
	function _load_360_view_product(){
		...
		$(".papzi-360-button").on( "click", function() {
	...
	function _click_button(){
		...
		$(".header-page-link .woocommerce-notices-wrapper").delay(4000).fadeOut();
	}
	function _after_add_to_cart(){
		...
		if(wd_width >= 991){
			$( _body ).on( 'added_to_cart', function(){
				_click_add_to_cart();
				$.ajax({
					url: papzi_ajax.ajaxurl,
					data: {
						"action" : "papzi_cartajax"
					},
	...
	function _click_add_to_cart(){
		...
		$(".cart-icon").on( "click", function() {
			if( !_body.hasClass("not-scroll") && $(".papzi-topcart").hasClass("popup") ){
			...
			$.ajax({
				...
				success: function(data){
					...
					if (data.loggedin == true){
						...
						document.location.href = papzi_ajax.redirecturl;
					}else{
	...
	class Elementor_Js_Papzi {
		static getInstance() {
			if (!Elementor_Js_Papzi.instance) {
				Elementor_Js_Papzi.instance = new Elementor_Js_Papzi();
			}
			return Elementor_Js_Papzi.instance;
		}
		constructor() {
	...
	Elementor_Js_Papzi.getInstance();	
